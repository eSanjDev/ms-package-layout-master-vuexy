...
/******* ConfirmActions (delete save ...) *******/
function ConfirmActions(
  e,
  element,
  {
    titleModal = 'Do You Want to delete?',
    textModal = 'You can also restore your code without any limitations from the history section to return to previous code changes.',
    textBtn = 'Yes Delete Record',
  ...
  if (element) {
    let classBtn = confirmclass === 'btn-delete' ? 'btn-delete' : 'btn-success';
    Swal.fire({
      title: titleModal,
      text: textModal,
      ...
      customClass: {
        confirmButton: `btn ${classBtn} me-2 waves-effect waves-light`,
      ...
      if (result.isConfirmed) {
        Swal.fire({
          title: confirmclass === 'btn-delete' ? 'Deleted!' : 'Saved!',
          text:
            confirmclass === 'btn-delete'
          ...
          const tablePosition = element.closest('tr');
          const form = element.closest('form');
          const tableServicesItem = element.closest('.table-services__item');

          if (tableServicesItem) {
            tableServicesItem.remove();
          }

          if (tablePosition) {
            tablePosition.remove();
          } else {
...
// menu active
function menuActive(event, menuItem) {
...
document.addEventListener('keydown', function (event) {
  if (event.key === 'Control') {
    let btnGuide = document.querySelectorAll('.guide-link');
    if (btnGuide.length > 0) {
      btnGuide.forEach(btn => {
...
// show Modal Guide
function showModalGuide(event, element) {
  event.preventDefault();
  let textGuide = element.getAttribute('data-tooltip-key');
  if (textGuide) {
    Swal.fire({
      title: 'guide box',
      text: textGuide,
      ...
      confirmButtonText: 'OK',
      customClass: {
        confirmButton: `btn btn-create me-2 waves-effect waves-light`
      },
      buttonsStyling: false
    });
  }
...
// copy code in textarea code mirror
function copyCodeMirror(event, element) {
  event.preventDefault();

  let i = -1;

  const getMessage = function () {
    const msgs = ['code copied. '];
    i = (i + 1) % msgs.length;
    return msgs[i];
  };
  ...
  let textareaCode = element.closest('.code-mirror-box').querySelector('textarea');
  if (textareaCode) {
    const originalText = textareaCode.value;
    navigator.clipboard.writeText(originalText);
    notyf.success(getMessage());
  }
}

/************** copy text (code or text or number ...) **************/
function copyText(event, element, title = 'Token') {
  event.preventDefault();
  ...
  let text = element.getAttribute('data-textCopy');
  console.log(text);
...
window.showHideSelectBox = function (element) {
    const container = element.closest('.card') || document;
    if (container) {
        const boxContent = container.querySelector('.box-content__form');
        const btnShowItem = container.querySelector('.btn-show-item');

        if (btnShowItem && boxContent) {
            if (btnShowItem.checked) {
